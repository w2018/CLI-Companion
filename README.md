...
- 🗂️ **服务管理**：新建/编辑/删除/启动/停止/重启，参数有序编辑（选项/开关/位置参数），环境变量（机密 DPAPI 加密存储、不同步、不写日志）；**粘贴整条命令行自动解析**为参数条目；服务克隆与拖拽排序
- 🖥️ **窗口语义**：无窗口 / 新控制台（可见·隐藏）三种控制台模式，按服务独立配置
- 🧩 **服务健康**：进程/自定义命令探活，内存告警阈值（超限系统通知）；崩溃自动重启（指数退避 + 10 分钟熔断），崩溃自动归档诊断包（退出码 + 日志尾部）
- 💾 **配置自动备份**：每次保存前自动快照（保留 20 份），设置页一键回滚
- 🖥️ **内嵌终端 / 调试终端**：GUI 内嵌 xterm 终端（ConPTY），6 套主题切换、自动换行开关、选中文本后右键菜单复制/粘贴（粘贴多行内容先弹窗确认）；返回页面会话保持、重新进入继续；也可用服务环境弹出独立控制台（Windows Terminal 优先）
- 🌐 **本机状态页**（可选）：127.0.0.1 只读展示服务状态、资源占用与运行统计，保存开关即时生效
...
- 📊 **资源监控**：每服务实时 CPU 占用（进程树合计，按逻辑核数归一化）、内存（含占系统百分比）、GPU 利用率与显存、磁盘读写速率、网络收发速率（TCP 口径），仪表盘与服务列表一目了然，数值按占用高低分档变色（正常/偏高/过载，明暗主题自适应）；无 GPU 的机器自动隐藏 GPU 项
- 🔔 **失败系统通知**：服务崩溃、自动重启失败或熔断时弹出 Windows 通知，关闭 GUI 也能第一时间知道
- 🖥️ **调试终端**：以该服务的环境变量与工作目录打开一次性控制台（不改系统环境变量），配置前后都能验证；一键复制完整启动命令行
- 💾 **配置导入导出**：一键导出/导入 JSON 配置备份，跨机器迁移无需手动拷贝
- 📜 **日志**：每服务独立日志，实时查看、10MB 轮转归档、一键清理日志内容；守护进程自身运行日志（daemon.log）可在「守护进程日志」页实时查看
- 🔁 **双进程架构**：daemon 常驻托管服务，GUI 随开随关；GUI 启动自动拉起 daemon（管道占用重试 + 并发拉起去重 + 单例锁健康探测，杜绝多实例）
- ☁️ **WebDAV 配置同步**：多设备协作，双向修改自动检测、本地优先（LWW）+ 冲突文件；凭据 DPAPI 加密存储
- ⚙️ **Win32 服务模式**：`--install-service` 一键注册系统服务，开机自启、无人值守
- 🔌 **开机自动启动**：三档模式（关闭 / 仅启动 daemon（默认）/ daemon + GUI），登录 Windows 后自动拉起守护进程并恢复自启服务，默认不显示 GUI 窗口
- 📦 **便携模式**：`--portable` 数据目录跟随 exe（写入 portable.marker），U 盘即插即用
- 🖥️ **系统托盘**：关闭最小化到托盘；托盘「服务」子菜单直接启停各服务并显示运行状态；退出方式可选（仅退出 GUI / 完全退出并停止服务）
- 🚀 **单实例**：重复启动自动唤起已有窗口
- 📁 **应用功能页 + 内置 FTP 服务端**（v2.6.0）：daemon 内置应用功能页统一管理工具型功能；已实现完整 FTP 服务端——多端口多站点（每个控制端口独立根目录）、多用户细粒度权限（浏览/下载/上传/删除/重命名/创建目录）、多目录授权（未授权的授权目录自动挂载为虚拟子目录）、被动/主动模式、DPAPI 加密密码存储、配置变更动态生效（用户/权限修改对新登录即时生效）
